...
> **Lineage.** Written against `arr-ai/wbnf` in February–April 2026. xbnf
> is the clean-slate implementation of this design. Surface-syntax sigils
> changed (`.` pragmas became `#`; `.` is any-character). Engine target
> is GLL plus a layered DFA fast path, with no PEG compatibility.
> Canonical language spec: [`xbnf.xbnf`](xbnf.xbnf). Implementation plan:
> [`plan.md`](plan.md).
